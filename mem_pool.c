...
alloc_pt mem_new_alloc(pool_pt pool, size_t size) {
    ...
    if(remaining_gap_size != 0) {
        ...
        (*un_node).alloc_record.size = remaining_gap_size;
        (*un_node).alloc_record.mem = (*a_node).alloc_record.mem + size;
        (*un_node).allocated = 0;
        (*un_node).used = 1;
        ...
        (*un_node).prev = a_node;
        (*un_node).next = (*a_node).next;
        if((*a_node).next != NULL) {
            (*(*a_node).next).prev = un_node;
        }
        (*a_node).next = un_node;
        // add to gap index
        _mem_add_to_gap_ix(pool_mgr, remaining_gap_size, un_node);
        // check if successful
    }
...
alloc_status mem_del_alloc(pool_pt pool, alloc_pt alloc)
{
    ...
    node_pt node_delete = (node_pt) alloc;
    // convert to gap node
    ...
    if((*node_delete).next != NULL && (*(*node_delete).next).allocated == 0) {
        // remove the next node from gap index
        _mem_remove_from_gap_ix(pool_mgr, (*(*node_delete).next).alloc_record.size, (*node_delete).next);
        // add the size to the node-to-delete
        (*alloc).size += (*(*node_delete).next).alloc_record.size;
        // update node as unused
        (*(*node_delete).next).used = 0;
        (*(*node_delete).next).alloc_record.size = 0;
        (*(*node_delete).next).alloc_record.mem = NULL;
        // update metadata (used nodes)
        (*pool_mgr).used_nodes--;
        // update linked list:
        // there exists a node after the next node
        if((*(*node_delete).next).next != NULL) {
            (*(*(*node_delete).next).next).prev = node_delete;
            (*node_delete).next = (*(*node_delete).next).next;
        }
            // the next node is the last node
        else {
            (*(*node_delete).next).prev = NULL;
    ...
    if((*node_delete).prev && (*(*node_delete).prev).allocated == 0) {
        node_pt prev_node = (*node_delete).prev;
        ...
        node_delete = prev_node;
    }
    // add the resulting node to the gap index
    alloc_status add_status = _mem_add_to_gap_ix(pool_mgr, (*node_delete).alloc_record.size, node_delete);
    // check success
    return add_status;
}
...
void mem_inspect_pool(pool_pt pool, pool_segment_pt *segments, unsigned *num_segments) {
    ...
    node_pt curr_node = (*pool_mgr).node_heap;
    int i = 0;
    // loop through the node heap and the segments array
    while(curr_node != NULL) {
        // for each node, write the size and allocated in the segment
        sgmts[i].size = (*curr_node).alloc_record.size;
        sgmts[i].allocated = (*curr_node).allocated;
        i++;
